...
public class CasseroleVision {
    ...
    private double  tgtRange = 8;
    private double  tgtPercent = 0;
     ...
     */
    public CasseroleVision(boolean useUSBStream) {
        int retry_counter = 0;
    ...
    public double getTgtPercent_in() {
        return tgtPercent;
    }
    /**
    ...
    public int parsePacket(String pkt, double rx_Time){
        ...
        String[] tokens = pkt.replaceAll("\\(", "").replaceAll("\\)","").split(",");
        if (tokens.length ==1) {
        	Targetvisible = false;
        }
        //Check there were enough substrings found
        else if(tokens.length < NUM_EXPECTED_TOKENS){
        ...
        try {
            ...
            tgtRange    = Double.parseDouble(tokens[distance]);
            tgtPercent  = Double.parseDouble(tokens[percent]);
            tgtAngleDeg = Double.parseDouble(tokens[angle]);
            //tgtRange = (tgtRange.substring(1, tgtRange.length()));
