...
public class Drivetrain {
	...
	public static final double SPROCKET_RATIO = 15.0 / 26.0; // 15 tooth sprocket on gearbox, 26 tooth sprocket on
																// wheels
	public static final double WHEEL_ROLLING_RADIUS_FT = 0.24; // ~6 inch pneumatic wheels with a bit of squish.
	...
	private Drivetrain() {
		...
		if (useRealGearbox) {
			leftGearbox = new RealGearbox(RobotConstants.CANID_DRIVETRAIN_LEFT_MASTER_SRX,
					RobotConstants.CANID_DRIVETRAIN_LEFT_SLAVE1_SRX, RobotConstants.CANID_DRIVETRAIN_LEFT_SLAVE2_SRX,
					"left");
			rightGearbox = new RealGearbox(RobotConstants.CANID_DRIVETRAIN_RIGHT_MASTER_SRX,
					RobotConstants.CANID_DRIVETRAIN_RIGHT_SLAVE1_SRX, RobotConstants.CANID_DRIVETRAIN_RIGHT_SLAVE2_SRX,
					"right");
		} else {
	...
	public void update() {
		...
		if (!isClosedLoop) {
			...
			leftCurrentEst_A = leftCurrentEst.getCurrentEstimate(leftGearbox.getMotorSpeedRadpSec(), leftMotorCommand,
					systemVoltage_V);
			rightCurrentEst_A = rightCurrentEst.getCurrentEstimate(rightGearbox.getMotorSpeedRadpSec(),
					rightMotorCommand, systemVoltage_V);
			...
			if (Gyro.getInstance().isOnline() && useHeadingCommand == true) {
				// Switch-mode gains since the in-motion correction is more aggressive than
				// stand still logic and causes instability
	...
	// 2016-style current limiter. Calculates an adjustment factor for drivetrain
	// commands based on predicted current draw.
	...
	private boolean isAcceptableVoltage(double leftOutput, double rightOutput) {
		...
		double leftCurEst = leftCurrentEst.getCurrentEstimate(leftGearbox.getMotorSpeedRadpSec(), leftOutput,
				systemVoltage_V);
		double rightCurEst = rightCurrentEst.getCurrentEstimate(rightGearbox.getMotorSpeedRadpSec(), rightOutput,
				systemVoltage_V);
