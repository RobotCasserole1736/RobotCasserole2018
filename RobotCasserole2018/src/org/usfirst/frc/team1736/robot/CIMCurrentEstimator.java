...
public class CIMCurrentEstimator {
	...
	//Guessed Constants
	private static final double motorWiringResistance = 0.131; //total guess to make the numbers work? Thhis accounts for errors in windings, wiring, connectors, etc.
	 ...
	 */
	public CIMCurrentEstimator(int numMotors, double controllerVDrop_V) {
	...
	public double getCurrentEstimate(double motorSpeed_radpersec, double motorCommand, double systemVoltage) {
		if(motorCommand > 0.05)
			return Math.max((numMotorsInSystem)*(((systemVoltage-contVDrop)*motorCommand)-Ki*motorSpeed_radpersec)/ESR, 0);
		else if(motorCommand < -0.05)
			return -Math.min((numMotorsInSystem)*(((systemVoltage-contVDrop)*motorCommand)-Ki*motorSpeed_radpersec)/ESR, 0);
		else
			return 0.0;
	}
