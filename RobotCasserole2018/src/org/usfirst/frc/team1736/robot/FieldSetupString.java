...
public class FieldSetupString {
	...
	public void update() {
		...
		try {
			...
			if (gameData.compareTo(prevGameData) != 0 && (left_Switch_Owned == true || right_Switch_Owned == true)
					&& (left_Scale_Owned == true || right_Scale_Owned == true)) {
				// New string, and some ownership was found. Good game data
				CrashTracker.logGenericMessage("got new game data: " + gameData);
			} else if (gameData.compareTo(prevGameData) != 0
					&& (left_Switch_Owned == false && right_Switch_Owned == false)
					&& (left_Scale_Owned == false && right_Scale_Owned == false)) {
