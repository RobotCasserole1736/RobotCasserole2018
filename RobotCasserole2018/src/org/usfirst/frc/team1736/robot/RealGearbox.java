...
public class RealGearbox implements Gearbox {
	...
	public RealGearbox(int canid1, int canid2, int canid3, String name) {
		...
		// produces positive sensor readings (in-phase) or negative sensor readings
		// (out-of-phase)
	...
	public double getTotalCurrent() {
		return motor1_current + motor2_current + motor3_current;
	}
	...
	// CTRE measures velocity in terms of "per 100ms" (???!?!) - hence factor of 600
	// to get to/from "per-minute"
	// CTRE does quadrature decoding of every edge, so each encoder full period is
	// counted as 4 pulses
	...
	// Other gains will have a timing scale factor (per 1ms???) not sure. We'll
	// scale them all the same for now.
	// CTRE will have the gain set in terms of output percentage steps per native
	// encoder units
