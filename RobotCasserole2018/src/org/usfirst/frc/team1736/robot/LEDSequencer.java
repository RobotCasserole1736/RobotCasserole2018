...
public class LEDSequencer {
	...
	private LEDSequencer() {
		...
		// Easy peasy check to see if weshould be running an actual strip or a fake
		// strip
		...
		timerThread.schedule(new LEDBackgroundUpdateTask(this), (long) (CasseroleLEDInterface.m_update_period_ms),
				(long) (CasseroleLEDInterface.m_update_period_ms));
	...
	private void smoothStripSweep() {
		...
		for (int led_idx = 0; led_idx < RobotConstants.NUM_LEDS_TOTAL; led_idx++) {
			double not_red_comp = Math.min(1,
					Math.max(0, (0.5 + edgeSharpness * Math.sin((led_idx / width + loop_counter / period)))));
	...
	private void cylon() {
		...
		double midpoint = (double) (Math.abs(((loop_counter) % period) - period / 2)) / ((double) (period / 2.0))
				* (RobotConstants.NUM_LEDS_TOTAL);
	...
	private void cometRed() {
		...
		double endpoint = (double) (((loop_counter) % period) / ((double) period))
				* ((RobotConstants.NUM_LEDS_TOTAL + width * 10) / 2.0);
	...
	private void cometRainbow() {
		...
		double endpoint = (double) (((loop_counter) % period) / ((double) period))
				* ((RobotConstants.NUM_LEDS_TOTAL + width * 10));
	...
	private void smoothBlueGreenSweep() {
		...
		for (int led_idx = 0; led_idx < RobotConstants.NUM_LEDS_TOTAL; led_idx++) {
			double not_green_comp = Math.min(1,
					Math.max(0, (0.3 + edgeSharpness * Math.sin((led_idx / width + loop_counter / period)))));
	...
	public void pickRandomPattern() {
		// cur_pattern =
		// LEDSwitchCase.values()[(int)(Math.random()*((double)(LEDSwitchCase.values().length)))];
		cur_pattern = LEDSwitchCase.SMOOTH_SWEEP;
	}
