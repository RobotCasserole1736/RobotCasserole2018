...
public class MotorCurrentEstimator {
	...
	//Guessed Constants
	private double motorWiringResistance = 0.131; //total guess to make the numbers work? This accounts for errors in windings, wiring, connectors, etc.
	
	//Derived motor constants
	private final double CIM_ESR = CIM_operatingVoltage/CIM_stallCurrent_A;
	private final double CIM_Ki = (CIM_operatingVoltage - CIM_freewheelCurrent_A*CIM_ESR)/CIM_freewheelSpeed_RadperSec;
	
	private final double MiniCIM_ESR = MiniCIM_operatingVoltage/MiniCIM_stallCurrent_A;
	private final double MiniCIM_Ki = (MiniCIM_operatingVoltage - MiniCIM_freewheelCurrent_A*CIM_ESR)/MiniCIM_freewheelSpeed_RadperSec;
     ...
     *      wiringResistance_ohm = voltage drop induced by the motor controller, in V. 
	 */
	public MotorCurrentEstimator(int numCIMMotors, int numMiniCIMMotors, double cmd_fudge_in, double speedGainFudgeFactor_in) {
	...
	public double getCurrentEstimate(double motorSpeed_radpersec, double motorCommand, double systemVoltage) {
		...
		if(motorCommand > 0.05) {
			CIMMotorCurrent = Math.max((numCIMMotorsInSystem)*(((systemVoltage)*motorCommand*Cmd_fudge)-Ki_fudge*CIM_Ki*motorSpeed_radpersec)/(motorWiringResistance + CIM_ESR), 0);
			miniCIMMotorCurrent = Math.max((numMiniCIMMotorsInSystem)*(((systemVoltage)*motorCommand*Cmd_fudge)-Ki_fudge*MiniCIM_Ki*motorSpeed_radpersec)/(motorWiringResistance + MiniCIM_ESR), 0);
		} else if(motorCommand < -0.05) {
			CIMMotorCurrent = -Math.min((numCIMMotorsInSystem)*(((systemVoltage)*motorCommand*Cmd_fudge)-Ki_fudge*CIM_Ki*motorSpeed_radpersec)/(motorWiringResistance + CIM_ESR), 0);
			miniCIMMotorCurrent = -Math.min((numMiniCIMMotorsInSystem)*(((systemVoltage)*motorCommand*Cmd_fudge)-Ki_fudge*MiniCIM_Ki*motorSpeed_radpersec)/(motorWiringResistance + MiniCIM_ESR), 0);
		} else {
			CIMMotorCurrent = 0.0;
		}
	...
	/**
	 * If there is need for some specific offset factor (not symmetrical wiring?) then this is how it can be added.
	 * @param r_in
	 */
