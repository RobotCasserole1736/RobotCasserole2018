...
public class ElevatorCtrl {
	...
	// Derived Constants
	public final double ELEV_HEIGHT_IN_PER_WINCH_REV = 2 * Math.PI * (ELEV_WINCH_DIAMETER_IN / 2.0); // Linear rope
																										// distance is
																										// 2*pi*r_winch
	...
	private ElevatorCtrl() {
		...
		AutoPilePosCal = new Calibration("Elev Auton Pile  position (in)", 8.0, 0.0, 84.0);
	...
	public void update() {
		...
		if (lowerTravelLimitReached != prevLowerTravelLimitReached) {
			// We've actuated the switch. Must be moving but right at the bottom of the
			// travel limit.
		...
		if (isZeroed == false) {
			...
			// Continuous mode - used whenever the driver wants control, or the encoder has
			// not yet been zeroed.
	...
	/**
	 * Conversion between an elevator height (from an enum value) to the presently
	 * configured height (in inches)
	 * 
	 * @param cmd
	 *            enum height command
	 * @return height, in inches, or -1 if the previous command should be
	 *         maintained.
	 */
