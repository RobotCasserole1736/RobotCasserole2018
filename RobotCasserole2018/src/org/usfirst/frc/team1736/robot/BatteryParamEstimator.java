...
/**
 * Battery parameter estimator - calculates an estimate for a battery's open
 * circuit voltage (Voc) and equivilant series resistance (ESR) based on a
 * windows of system current/voltage measurements. Ensures enough spread in the
 * measurement window to ensure confidence in the estimate. Based on a
 * whitepaper detailing an algorithm developed for the 2016 FRC season by
 * FRC1736 RobotCasserole.
 * 
 * @author Chris Gerth
 *
 */
public class BatteryParamEstimator {
	...
	/**
	 * updateEstimate - Update the internal estimates with a new measured system
	 * voltage and current. Should be called once per control loop.
	 * 
	 * @param measSysVoltage_V
	 *            - Battery voltage as measured by PDB
	 * @param measSysCurrent_A
	 *            - Current draw as measured by PDB
	 */
	public void updateEstimate(double measSysVoltage_V, double measSysCurrent_A) {
		...
		// If the spread is above the preset threshold, we will be confident for this
		// window
		...
		} else { // If the spread is too small, we're not confident, and reset previous best
					// spread.
			confident = false;
			prev_best_spread = 0;
		}
	 ...
	 * 
	 * @return True if the most recent window had enough spread for a reasonable
	 *         estimate, false if not.
	 */
	...
	/**
	 * getEstVsys - given a system current draw, estimate the resulting system
	 * voltage given the battery parameters.
	 * 
	 * @param Idraw_A
	 *            - estimated system current draw in Amps
	...
	/**
	 * getMaxIdraw - given a minimum system voltage allowable, estimate the maximum
	 * current which may be drawn from the battery at present time..
	 * 
	 * @param VsysMin_V
	 *            - minimum desireable system voltage
	...
	/**
	 * findDotProd - calculates the dot product of two equally-sized arrays of
	 * doubles A and B. Dot product is found by multiplying each element of A with
	 * the corresponding element of B, and adding together the result of each
	 * multiplication.
	 * 
	 * @param A
	 *            First Array
	...
	/**
	 * findStdDev - Calculates the standard deviation of a set of doubles. Standard
	 * deviation is defined as the square root of the sum of the squares of the
	 * distances of each element from the average of the dataset.
	 * 
	 * @param input
	 *            - doubles to take the Standard Deviation of.
