...
/**
 * DESCRIPTION: <br>
 * Realtime Plot View webpage definition class. Allows the user to send a set of
 * variables to an interface where they can be plotted in real-time. <br>
 * ASSUMPTIONS: <br>
 * Be sure the casserole webserver is started at some point, otherwise the
 * webpage won't be displayed. Note that states can only be added (even at
 * runtime), but never removed. At least not yet... <br>
 * USAGE:
 * <ol>
 * <li>On init, call addNewSignal with all the signals you want available.</li>
 * <li>During runtime, call the addSample() method to record a new value to be
 * plotted.</li>
 * </ol>
 * 
 *
...
public class CasseroleWebPlots {
	/**
	 * The set of objects which are broadcast. Must be volatile to ensure atomic
	 * accesses
	 */
	...
	/**
	 * Put a new signal to the web interface, or update an existing one with the
	 * same name
	 * 
	 * @param name
	 *            Name for the state to display.
	 * @param value
	 *            Double Floating-point value to display
	 */
	...
	/**
	 * Given an array of signal names from a client, starts acquisition of those
	 * signals
	 * 
	 * @param signal_names
	 */
